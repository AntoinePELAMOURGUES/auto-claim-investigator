...
B -->|Secure Upload| C[(Snowflake Internal Stage)]
    ...
    E -->|Snowflake CORTEX (Mistral-Large)| F[AI ANALYSIS Layer]
...
Résultat de la transformation : | Input (Non structuré) | Output IA (Structuré) | | :--- | :--- | | "Je suis sorti de mon stationnement et j'ai heurté le véhicule B." | 100% Responsable | | "J'étais à l'arrêt au feu rouge, on m'a percuté par l'arrière." | 0% Responsable |
...
# 1. Générer et Ingérer les données (Python)

cd scripts
python generate_fake_claims.py
python ingest_data.py

# 2. Transformer et Analyser (dbt)

cd ../dbt_claims
dbt run
📊 Documentation & Lineage
La documentation technique et le lignage des données sont générés automatiquement.

(Insérer ici la capture d'écran du Lineage Graph dbt)
